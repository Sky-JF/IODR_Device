...
// Blank Display
// display current blank value for all tubes on OLED
void displayTubeBlankValues() { // %%%
  char displayLine[4][17] = {
       "1:      5:      ",
       "2:      6:      ",
       "3:      7:      ",
  ...
  //add in raw values to first column
  for (int i=0; i<numTubes/2; i++){  
    //loop through each character in odval and add it to the displayLine char array
    //add rawVal in first position (i.e. tubes 1, 2, 3 and 4)
    snprintf(rawVal, sizeof(rawVal), "%4d", (int)lightIn[i]);
    
    for (int j = 0; j<sizeof(rawVal)-1; j++){ //sizeof(blankVal)-1 is to avoid including the "end of string" character
      displayLine[i][j+2] = rawVal[j];
    }
    //add rawVal in second position (i.e. tubes 5, 6, 7 and 8)
    snprintf(rawVal, sizeof(rawVal), "%4d", (int)lightIn[i+4]);
    for (int j = 0; j<sizeof(rawVal)-1; j++){
      displayLine[i][j+10] = rawVal[j];
    }
  }
...
void  uploadDataToThingspeak() {
  ...
  String odBody = client.responseBody();
  ...
  delay(500); // test rate_limits, this seems to improve connectivity *** %%%
...
void connectToWifi() {
  ...
  if (WiFi.status() == WL_NO_MODULE) { 
    ...
    // don't continue
    while (true); // wait until watch dog timer resets the program
  }
...
// Check for the first 5 seconds of the program if the blank values should be caibrated
// If the calibration pin is pressed in this window, the blank values of tubes are shown until the calibration pin is pressed again
// Press tube button 8 to exit this window early
void potCalibrationCheck() 
{
  int currTime = millis();
  bool activatePotCalibration = false;
  Serial2 << gloClear << "Press calibrate" << gloReturn << "button to" << gloReturn << "enter calibrate" << gloReturn << "mode";
  while (millis() < currTime + POT_CALIBRATION_WINDOW) {
    watchdog.kick();

    if (!digitalRead(calibrationPin)) {
      activatePotCalibration = true;
      Serial.println("Entering pot calibration mode");
    } else if (!digitalRead(blankButtonPin[7])) {
      break; // exit the loop if tube 8 button is pressed to go straight to data collection
    }

    // Enter calibration mode until the button is pressed again to exit
    if (activatePotCalibration) {
      Serial.println("Stop pressing calibration button\nPress calibration button again to exit");
      Serial2 << gloClear << "Calibrate mode" << gloReturn << "!Stop pressing!" << gloReturn << "Press button" << gloReturn << "again to exit";
      delay(3000);
      do {
        watchdog.kick();
        readLightSensors();
        displayTubeBlankValues();
        delay(500); // wait 0.5 seconds before measuring raw values again
      } while (digitalRead(calibrationPin)); // press button to exit
      break;
    }
  }
}
