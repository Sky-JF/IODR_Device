...
void setup(void) {
  ...
  if (watchdog.start(WATCHDOG_TIMEOUT)) {
  ...
  Serial.println("==========================");
  Serial2 << gloClear << "Internet OD" << gloReturn << "reader v" << VERSION << " ID:" << IODR_ID;
  watchdog.kick();
  ...
  connectToWifi();
  Serial2 << gloClear << "Connected wifi";
  delay(1000);

  /* pot-calibration mode
   * if the calibrationPin button is pressed in the first 10 seconds after the program is started, then the calibration mode is activated
   * if the calibration mode is activated, press the calibrationPin button again to exit it and start normal program function
   */
  potCalibrationCheck();
